<?php
class ResizeImageShell extends AppShell{
	 var $uses = array('ArticleAsset','Article');
	 
	 public function main() {
        $rs = $this->Article->query("SELECT filename FROM article_assets;");
		foreach($rs as $r){
			$path = getcwd()."/webroot/";
			$this->process($r['article_assets']['filename']);
	 ...
	 private function process($filename){
		...
		$path = getcwd()."/webroot/";
	 	if($this->resizeImage('resizeCrop', $filename, 
			...
			$this->resizeImage('resize', $filename, 
										"{$path}content/images/", 
										"big_".$filename, 
										980, 425, 80);
