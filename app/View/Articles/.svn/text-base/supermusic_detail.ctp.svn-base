...
<script type="application/javascript">
	function postToFeed() {
        ...
        var obj = {
          method: 'feed',
          link: '<?php echo $domain.$this->Html->url("/articles/{$cat['Channel']['name_str']}/view/{$article['id']}");?>',
        ...
        function callback(response) {
          $.ajax({
				  url: '<?php echo $this->Html->url("/articles/{$cat['Channel']['name_str']}/view/{$article['id']}");?>'+'?ajax=1&share_type=2&post_id='+response['post_id']+'&tc=<?php echo "{$tc}";?>',
				  dataType: 'json',
