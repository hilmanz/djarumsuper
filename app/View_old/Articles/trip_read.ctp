<?php
$permalink = "/articles/trip/{$province['name_str']}/{$category}/view/{$article['id']}";
?>
    ...
    <div id="content">
        ...
        <div class="paper">
            <div class="content">
                ...
                <div class="social">
                	<div class="twitThis">
                        <a href="https://twitter.com/share" class="twitter-share-button" data-url="<?php echo $this->Html->url($permalink, true ); ?>" data-via="djarum" data-hashtags="djarum">Tweet</a>
                    ...
                    <div class="fbLike">
                        ...
                        <script type="application/javascript">
                        	function postToFeed() {
						        ...
						        var obj = {
						          method: 'feed',
						          link: '<?php echo $domain.$this->Html->url($permalink);?>',
						        ...
						        function callback(response) {
						          $.ajax({
										  url: '<?php echo $this->Html->url($permalink);?>'+'?ajax=1&share_type=2&post_id='+response['post_id']+'&tc=<?php echo "{$tc}";?>',
										  dataType: 'json',
                    ...
                    <div class="emailBox">
                    	<?php $article_link = $crypto->urlencode64($this->Html->url($permalink,true));?>
                    	<a href="<?php echo $this->Html->url("/share/send/{$article['id']}?r=".$article_link,true);?>" class="emailIcon">&nbsp;</a>
                    </div>
                </div><!-- end .social -->
                <div class="social">
                	<div class="fb-comments" data-href="<?php echo $this->Html->url($permalink, true ); ?>" data-num-posts="5" data-width="470"></div>
                </div>
            </div><!-- end .paper -->
        </div><!-- end .content -->
    </div><!-- end #content -->
		  ...
		  <div class="moreArticle">
		  	<div class="widgetContent">
		        ...
		        <div class="entry">
					...
					?>
		            <div class="row">
		                <p><?php echo $this->Html->link("{$f['Article']['title']}","/articles/trip/{$province['name_str']}/{$category}/view/{$f['Article']['id']}");?></p>
  ...
  function onTweet(intent_event) {
    if (intent_event) {
      ...
      //console.log("tweet nih");
       $.ajax({
		  url: '<?php echo $this->Html->url($permalink);?>'+'?ajax=1&share_type=1&post_id='+intent_event.target.baseURI+'&tc=<?php echo "{$tc}";?>',
		  dataType: 'json',
		  success: function( response ) {
			//console.log("share tracked");
		  }
